...
const char* CAR_PARKED_MESSAGE = "Car parked";
const char* CAR_LEFT_MESSAGE = "Car left";
const char* MQTT_TOPIC_HEALTHCHECK = MQTT_TOPIC_PREFIX "health";
const char* MQTT_TOPIC_STATUS = MQTT_TOPIC_PREFIX "status";
const char* MQTT_TOPIC_EVENT = MQTT_TOPIC_PREFIX "event";
...
void mqtt_publish_event(esp_mqtt_client_handle_t client,
...
void serialize_event(char* buffer,
                    const EventType event,
                    const time_t timestamp) {
  snprintf(buffer, MQTT_MSG_BUFFER, "event: %s\ntimestamp: %lld",
          event == CAR_PARKED ? CAR_PARKED_MESSAGE : CAR_LEFT_MESSAGE,
          timestamp);
}
