...
float get_sensor_dist(const ultrasonic_sensor_t* sensor, const ssd1306_config_t* config, oled_display_t* oled_display, i2c_handler_t* i2c_handler)
{
	...
	for (int i = 0 ; i < 10; i++)
	    {
	        ...
	        show_text(config, oled_display, i2c_handler, 4, buffer); //3 is display page number
...
void main_loop(void *pvParameters)
{
    ...
    float dist_first = get_sensor_dist(&sensor, &config, &oled_display, &i2c_handler);
    printf("distance first: %0.04f cm\n", dist_first);


    //second sensor distance to the wall
    float dist_second = get_sensor_dist(&sensor2, &config, &oled_display, &i2c_handler);
    ...
    //now that distances are measured we can start checking for anomalies in measurements
    clear_screen(&config, &oled_display, &i2c_handler);
